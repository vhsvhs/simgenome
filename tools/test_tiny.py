...
if sys.argv.__len__() < 2:
    print "USAGE:"
    print "  $> python test_tiny.py OUTDIR"
    print ""
    exit()
...
def get_command(psammu, ursmu, rep):
    ...
    command += "--psampath " + OUTDIR + "/test_tiny.psam "
    command += "--urspath " + OUTDIR + "/test_tiny.urs "
    command += "--rulepath " + OUTDIR + "/test_tiny.rules "
    command += "--maxgen 2 "
    command += "--niid 10000 "
    command += "--popsize 1 "
...
def print_psams():
    fout = open(OUTDIR + "/test_tiny.psam", "w")
...
def print_urss():
    fout = open(OUTDIR + "/test_tiny.urs", "w")
    fout.write(">0\n")
    fout.write("AAAAA\n")
    fout.write(">1\n")
    fout.write( get_rand_nt(10) + "\n")
    fout.write(">2\n")
    fout.write( get_rand_nt(10) + "\n" )
    fout.write(">3\n")
    fout.write("AAAA\n")
...
def print_rules():
    fout = open( OUTDIR + "/test_tiny.rules", "w" )
...
psammu = 0.1
ursmu = 0.01
make_directory(psammu, ursmu, 0)
c = get_command(psammu, ursmu, 0)
print c
os.system(c)
