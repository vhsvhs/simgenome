"""
Tests for robustness of expression estimation given
variable IID sampling densities.
"""
...
def get_command(id = "", random = False, popsave = False, popsize = 2, randseed = 12345, pe_scalar=0.001, niid=10000):
    OUTDIR = os.path.abspath( RUNID + id )
    command = "simreg "
    command += " --outdir " + OUTDIR
    command += " --maxgen 1 "
    command += " --niid " + niid.__str__()
...
def get_error(niid):
    thisid = ".niid" + i.__str__()
    outdir = os.path.abspath( RUNID + thisid )
    
    p = outdir + "/FITNESS/fitness.gen0.txt"
    if (False == os.path.exists(p) ):
        return
    fin = open( outdir + "/FITNESS/fitness.gen0.txt", "r")
    errs = []
...
niids = [10,100,1000,5000,10000,50000,100000,500000]
for i in niids:
    thisid = ".niid" + i.__str__()
    OUTDIR = os.path.abspath( RUNID + thisid )
    ...
    commands.append( get_command(id = thisid, popsize=20, randseed=123456789, pe_scalar=0.005, niid=i) )
