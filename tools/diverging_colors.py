...
gene_color = {}
def init_colors(ngenes):
    for i in range(0, ngenes):
        gene_color[i] = (i+1).__str__()
#     RGB1 = np.array([59, 76, 192])
#     #RGB1 = np.array([1, 100, 1])
#     RGB2 = np.array([180, 4, 38])
#     colormap = ColorMapCreator(RGB1, RGB2,numColors=ngenes)
#     count = 0
#     print colormap.Xn, colormap.Yn, colormap.Zn
#     for c in colormap.colors:
#         gene_color[count] = "#%02x%02x%02x" % (c[0]*255, c[1]*255, c[2]*255)
#         count += 1
    ...
    def __init__(self, RGB1, RGB2, numColors = 33., divide = 255., 
        ...
        if filename == "":
              for c in colorMap:
                      ...
                      self.colors.append( (c[0], c[1], c[2]) )
        else:
